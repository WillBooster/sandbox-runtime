...
- `network.allowedResolvedAddresses` - Carve-outs that win over the denied set, for setups where an allow-listed hostname legitimately points at an otherwise-denied address — e.g. `["127.0.0.1"]` when dev hostnames map to a local server via `/etc/hosts`.
